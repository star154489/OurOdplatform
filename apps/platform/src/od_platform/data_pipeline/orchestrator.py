...
class DatasetPipeline:
    ...
    def run(self) -> dict:
        ...
        with tempfile.TemporaryDirectory(prefix=f"odp_{self.dataset_name}_") as tmp:
            ...
            report = analyze_class_balance(
                labels_per_image, classes, self.train_rate, self.val_rate,
            )
            ...
            fingerprint = self._compute_fingerprint(manifest)
        ...
        self._write_split_report(manifest, classes, labels_per_image, fingerprint)
        logger.info("流水线完成: 划分=%s, yaml=%s", counts, self.yaml_out)
        return {"counts": counts, "yaml": str(self.yaml_out), "classes": classes}

    @staticmethod
    def _compute_fingerprint(manifest: SplitManifest) -> str:
        """计算数据指纹:所有 split/stem 排序后取 SHA256 前 16 位。"""
        fp_lines: List[str] = []
        for name in ("train", "val", "test"):
            for img, _ in getattr(manifest, name, []):
                fp_lines.append(f"{name}/{img.stem}")
        fp_lines.sort()
        return hashlib.sha256("\n".join(fp_lines).encode()).hexdigest()[:16]

    def _write_split_report(self, manifest: SplitManifest, classes: List[str],
                            labels_per_image: Dict[str, List[str]],
                            fingerprint: str) -> None:
        """生成划分报告,写入 processed/<dataset>/split_report.json。"""
        splits = {"train": manifest.train, "val": manifest.val, "test": manifest.test}

        # 收集各 split 的文件列表 + 类别分布
        split_files: Dict[str, List[str]] = {}
        split_class_counts: Dict[str, Dict[str, int]] = {}
        for name, pairs in splits.items():
            stems = [img.stem for img, _ in pairs]
            split_files[name] = sorted(stems)
            class_counts: Dict[str, int] = {}
            for img, _ in pairs:
                for cls_name in labels_per_image.get(img.stem, []):
                    class_counts[cls_name] = class_counts.get(cls_name, 0) + 1
            split_class_counts[name] = dict(sorted(class_counts.items()))
        ...
        report = {
            ...
            "classes": classes,
            "fingerprint": fingerprint,
            "counts": {name: len(p) for name, p in splits.items()},
            "class_distribution": split_class_counts,
            "files": split_files,
        }

        report_path = self.processed_root / "split_report.json"
        report_path.parent.mkdir(parents=True, exist_ok=True)
        report_path.write_text(json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8")
        logger.info("划分报告已写入: %s (指纹: %s)", report_path, fingerprint)

    def _check_raw(self) -> None:
        """预检查:原始数据目录存在、有图、覆盖率达标。"""
        if not self.raw_root.is_dir():
            raise FileNotFoundError(f"数据集目录不存在: {self.raw_root}")
        # 弹性目录名: 兼容 images/JPEGImages 和 annotations/Annotations
        self.raw_images = self._resolve_dir(self.raw_root, ["images", "JPEGImages"])
        self.raw_annotations = self._resolve_dir(self.raw_root, ["annotations", "Annotations"])
        self._check_coverage()

    @staticmethod
    def _resolve_dir(parent: Path, candidates: List[str]) -> Path:
        """在 parent 下尝试多个候选子目录名,返回第一个存在的。"""
        for name in candidates:
            d = parent / name
            if d.is_dir():
                return d
        raise FileNotFoundError(f"{parent} 下缺少目录,尝试过: {candidates}")

    def _check_coverage(self) -> None:
        """检查图像-标注覆盖率(硬阈值:终止;软阈值:警告)。"""
        ...
        coverage = n_annos / n_images if n_images else 0.0
    ...
    def _pair_images_with_labels(self, labels_dir: Path) -> PairList:
        ...
        logger.info("配对: %d 对 (图 %d, 标注 %d)",
                     len(pairs), len(image_index), len(list(labels_dir.glob("*.txt"))))
        return pairs

    @staticmethod
    def _build_labels_per_image(pairs: PairList, classes: List[str]) -> Dict[str, List[str]]:
        """从已配对的列表还原 {图像stem: [类别名]}。

        读的是 YOLO txt(只有 class_id),用 classes 列表映射回类名。
        """
