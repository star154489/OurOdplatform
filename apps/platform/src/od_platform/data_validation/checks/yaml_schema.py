...
# @Function  : yaml_schema check — 验证 yaml 字段完整性和一致性
"""验证数据集 yaml 文件的字段完整性和一致性 — 消费 snapshot 预解析数据。"""
...
def validate_yaml_schema(ctx: CheckContext) -> CheckResult:
    ...
    if problems:
        return CheckResult(
            ...
            details={"reason": "field_inconsistency", "problems": problems,
                     "nc": nc, "names_count": len(names) if names else 0},
        )
    ...
    return CheckResult(
        ...
        details={"nc": nc, "names_count": len(names)},
    )
