...
def render_to_logger(
    ...
    _render_footer(report_path or report.report_path, logger)
...
def _render_one_check_details(r: CheckResult, logger: logging.Logger) -> None:
    ...
    elif r.name in ("class_presence", "split_presence") and det.get("findings"):
        for f_item in det["findings"]:
            logger.info(f"        [{f_item['severity']}] {f_item['message']}")
