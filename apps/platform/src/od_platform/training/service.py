...
class TrainService:
    ...
    def train(
        ...
        *,
        ...
        try:
            # 阶段1： 配置加载
            cli_ns = Namespace(**cli_args) if cli_args else None
            ...
            # 模型加载
            # Windows: 页面文件小时 CUDA DLL 批量加载会失败(1455)
            import os as _os
            ...
            try:
                ...
                if hasattr(e, 'winerror') and e.winerror == 1455:
                    logger.error(
                        "Windows 页面文件不足, 无法加载 CUDA DLL.\n"
                        "  请执行以下任一步骤后重试:\n"
                        "  1. 重启电脑 (最可靠)\n"
                        "  2. 管理员 PowerShell 执行:\n"
                        "     wmic pagefileset set InitialSize=16384,MaximumSize=32768\n"
                        "  3. 或关闭其他占用 GPU 的程序后重试"
                    )
                raise
            ...
            # Windows 多进程 DataLoader 需要大量页面文件，默认关掉
            if _platform.system() == "Windows":
                yolo_kwargs.setdefault("workers", 0)
