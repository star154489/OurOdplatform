...
# ── 数据集路径(供 data_pipeline 使用) ─────────────────────────
def dataset_processed_dir(name: str) -> "Path":
    """按数据集名分桶的落盘根,如 data/processed/safety_helmet_yolo/。"""
    return PROCESSED_DATA_DIR / f"{name}_yolo"


def dataset_yaml_path(name: str) -> "Path":
    """数据集 yaml 配置路径,如 configs/datasets/safety_helmet.yaml。"""
    return CONFIGS_DIR / "datasets" / f"{name}.yaml"
